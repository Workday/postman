...
/**
 * A helper class that runs validation on classes or fields annotated with {@literal@}{@link Parceled} and extracts all
 * valid fields eligible to be included in the Parcel.
 *
...
class ParceledElementExtractor {
    ...
    /**
     * Returns {@code true} if the given method is valid for the {@link PostCreateChild} annotation, and {@code false}.
     * If the method is not valid, this method will generate all the relevant compilation errors.
     */
    private boolean assertValidPostCreateMethod(ExecutableElement method) {
        ...
        if (method.getParameters().size() != 1 || !metatypes.isSameType(method.getParameters().get(0).asType(),
            ...
            String message = String.format(Locale.US,
                                           "Method annotated with @%s must take a single argument of type Object.",
                                           PostCreateChild.class.getSimpleName());
            processingEnv.getMessager().printMessage(Diagnostic.Kind.ERROR, message, method);
        }
    ...
    /**
     * Extracts all fields that should be included in the parcel. If {@code classToParcel} is annotated with
     * {@literal@}{@link Parceled}, then all fields except those that are final, private, or annotated with
     * {@literal@}{@link NotParceled} will be included. Otherwise, all fields annotated with {@literal@}Parceled are
     * included.
     *
     * @param classToParcel The element representing the class that will be parceled.
     *
        ...
        for (VariableElement field : allFields) {
            ...
            if (CodeAnalysisUtils.isConstant(field) || hasNotParceledAnnotation(field)) {
                ...
                if (CodeAnalysisUtils.isPrivate(field)) {
                    String message = String.format("Parceler will ignore private field %s.", field.getSimpleName());
                    messager.printMessage(Diagnostic.Kind.WARNING, message, field);
                    shouldAdd = false;
                }

                if (CodeAnalysisUtils.isFinal(field)) {
                    String message = String.format("Parceler will ignore final field %s.", field.getSimpleName());
                    messager.printMessage(Diagnostic.Kind.WARNING, message, field);
                    shouldAdd = false;
                }
                ...
                if (hasParceledAnnotation(field)) {
                    String message = String.format(
                            "@%s annotations are ignored when the enclosing class is annotated with @%s.",
    ...
    private Collection<VariableElement> extractAnnotatedFields(Collection<VariableElement> allFields) {
        Collection<VariableElement> validAnnotatedFields = new ArrayList<VariableElement>(allFields.size());
        for (VariableElement field : allFields) {
            boolean shouldAdd = hasParceledAnnotation(field);

            if (hasNotParceledAnnotation(field)) {
                String message = String.format(
                        "@%s annotations are ignored when the enclosing class is not annotated with @%s",
                        NotParceled.class.getSimpleName(), Parceled.class.getSimpleName());
                messager.printMessage(Diagnostic.Kind.WARNING, message, field);
            }

            if (hasParceledAnnotation(field) && CodeAnalysisUtils.isPrivate(field)) {
                String message = String.format("Cannot access private fields when parceling.");
                messager.printMessage(Diagnostic.Kind.ERROR, message, field);
                shouldAdd = false;
            }

            if (hasParceledAnnotation(field) && CodeAnalysisUtils.isFinal(field)) {
                String message = String.format("Cannot access final fields when parceling.");
                messager.printMessage(Diagnostic.Kind.ERROR, message, field);
                shouldAdd = false;
            }
