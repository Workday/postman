...
public class MapBundler {
     ...
     * @param bundleKey The string used to identify the map for later retrieval (see {@link
     * Bundle#get(String)}).
     * @param <K> The type for the map keys.
     * @param <V> The type for the map values.
     */
    public static <K, V> void writeMapToBundle(Map<K, V> map,
                                               Bundle bundle,
                                               Class<K> keyClass,
                                               Class<V> valueClass,
                                               String bundleKey) {
        ...
        ArrayListBundler.writeArrayListToBundle(values,
                                                bundle,
                                                valueClass,
                                                bundleKey + VALUES_SUFFIX);
    }

    /**
     * Read map stored in the bundle using the given {@code bundleKey}, populating {@code map} with
     * the result.
     *
     ...
     * @param <V> The type for the map values.
     */
    public static <K, V> void readMapFromBundle(Map<K, V> map,
                                                Bundle bundle,
                                                Class<K> keyClass,
                                                Class<V> valueClass,
                                                String bundleKey) {
        ArrayList<K> keys =
                ArrayListBundler.readArrayListFromBundle(bundle, keyClass, bundleKey + KEYS_SUFFIX);
        ArrayList<V> values = ArrayListBundler.readArrayListFromBundle(bundle,
                                                                       valueClass,
                                                                       bundleKey + VALUES_SUFFIX);
        ...
        final String message = String.format(Locale.US,
                                             "Expected keys.size() (%d) and values.size() (%d) to"
                                                     + " be the same.",
                                             keys.size(),
                                             values.size());
        Preconditions.checkState(keys.size() == values.size(), message);
